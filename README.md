...
사용한 DC 모터가 저토크 사양이라, 초기 PWM 듀티로는 알약 이송 중 동작 실패
PSC·ARR 기본값이 낮은 PWM 주파수를 생성해, 모터가 간헐적으로 멈췄다 가는 동작과 가청 소음(코일 진동음) 발생
스텝·서보·DC 모터 동시 구동 시 순간 전류로 전압 강하 발생 → MCU 리셋 반복

#### 원인 분석

모터 자체가 저토크 사양이라 낮은 듀티에서는 부하를 이기지 못함
PWM 주파수가 가청 대역에 걸려 코일이 진동(소음)하고, 모터 관성이 ON/OFF를 평균화하지 못해 끊김 발생
단일 전원으로 다수 구동계에 전류 공급 → 피크 전류 초과로 전압 강하, MCU 리셋 유발

#### 해결 방법

PWM 듀티를 단계적으로 테스트해 충분한 토크가 나오는 값(50%)으로 설정
PSC ARR을 조정해 PWM 주파수를 가청 대역 위로 높여 끊김·소음 없이 매끄럽게 회전하도록 함
외부 전원 모듈을 추가해 MCU와 구동계 전원을 분리

#### 결과

알약 배출·컨베이어 벨트 동작 안정화, 끊김·소음 없는 매끄러운 구동 및 연속 동작 안정성 검증 완료
...
#### 원인 분석
정지 상태의 모터를 가속 구간 없이 즉시 정속 구동할 경우 기동 스텝 누락이 발생한다. 이는 첫 구동 신호에 동기화되는 과정에서 관성에 의한 반응 지연이 발생하고, 이로 인해 초기 제어 스텝이 실제 회전력으로 이어지지 못한 채 손실된다.
...
#### 증상
RTC 알람 발생과 USART2 수신이 거의 동시에 발생했을 때, 어느 인터럽트가 먼저 처리될지 보장할 수 없어 알람 시퀀스 진입이 지연되는 경우가 있었다.

#### 원인
NVIC 우선순위를 명시하지 않으면 모든 인터럽트는 기본값(0)으로 동일 우선순위를 갖는다. 동시에 여러 IRQ가 pending 상태가 되면 하드웨어는 IRQ 번호가 낮은 것부터 처리하게 되어, 기능적 중요도와 처리 순서가 맞지 않을 수 있다.

#### 해결
`NVIC_SetPriorityGrouping(3)`으로 4비트 우선순위 비트를 전부 선점 우선순위에 할당한 뒤, 기능 중요도에 따라 우선순위를 명시했다.

```c
NVIC_SetPriorityGrouping(3);          // 4비트 전부 선점 우선순위로 사용
NVIC_SetPriority(41, 1);              // RTC Alarm  — 복약 알람, 최우선 처리
NVIC_SetPriority(40, 2);              // EXTI15_10  — 모드 전환
NVIC_SetPriority(38, 3);              // USART2     — 블루투스 데이터 수신
```

또한 각 인터럽트 핸들러는 플래그(`pill_alarm_flag`, `uart2_rx_exist`)만 세우고, 실제 시퀀스 처리는 메인 루프에서 수행하도록 ISR을 최소화했다.

#### 결과
RTC 알람이 항상 가장 먼저 처리되어 복약 시퀀스 진입 타이밍이 보장되고, ISR을 짧게 유지하는 구조로 다른 인터럽트의 응답성도 함께 확보했다.
